...
namespace TrackRadar
{
    internal static class PositionCalculator
    {
        ...
        public static bool IsOnTrack<T>(T point, IReadOnlyList<GpxTrackSegment> trackSegments,
        ...
        internal static bool IsOnTrack<T>(T point, IGeoMap<Segment> map, Length offTrackDistance, out double dist)
             where T : IGeoPoint
        {
            bool res = map.IsWithinLimit(point, offTrackDistance, out Length d);
            if (res)
                dist = -d.Meters;
            else
                dist = d.Meters;
